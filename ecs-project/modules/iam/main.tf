...
# Secret Manager access for task execution role
resource "aws_iam_role_policy" "secrets_manager_policy" {
  count = var.enable_rds_access && length(var.rds_secret_arns) > 0 ? 1 : 0
...
# S3 access policy (optional - only if application needs S3)
